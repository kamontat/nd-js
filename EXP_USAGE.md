...
$ nd --verbose [0,1,2,3] # 0 is mute and 3 is very verbose log message
...
```bash
$ nd [setting|s] init                   # open prompt with require information
$ nd [setting|s] init --json <json>     # load from json string
$ nd [setting|s] init --path <path>     # load from file path
$ nd [setting|s] init --replace         # replace new file with backup
```
...
$ nd [setting|s] update <json> --json   # update config with key and value (json format)
```
