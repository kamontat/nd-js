...
try {
  (async () => {
    ...
    if (!command.onlyMain) await execa("cp", ["-rf", path.join(bin, ndAdminName), path.join(resultBin, "nd-admin")]);
    if (!command.onlyAdmin) await execa("cp", ["-rf", path.join(bin, ndName), path.join(resultBin, "nd")]);
  })();
} catch (e) {
  console.error(e);
}
