...
if (!fs.existsSync(DIR_ND_ROOT_PATH)) fs.mkdirSync(DIR_ND_ROOT_PATH, { recursive: true });

export const DIR_TMP_NAME = "tmp";
export const DIR_TMP_PATH = path.join(DIR_ND_ROOT_PATH, DIR_TMP_NAME);
if (!fs.existsSync(DIR_TMP_PATH)) fs.mkdirSync(DIR_TMP_PATH, { recursive: true });

export const DIR_TMP_RANDOM = fs.mkdtempSync(path.join(DIR_TMP_PATH, "nd-"));
if (!fs.existsSync(DIR_TMP_RANDOM)) fs.mkdirSync(DIR_TMP_RANDOM, { recursive: true });
...
if (!fs.existsSync(DIR_LOG_PATH)) fs.mkdirSync(DIR_LOG_PATH, { recursive: true });
